...
| **4 — Trust & content** | Reviews, content + policy pages, FAQ, SEO, structured data, analytics | **Mostly done** — all six policy pages, ten content pages, 30-question FAQ, robots, sitemap, structured data, consent-gated analytics, 404/500. Remaining: review submission + admin moderation, the About page in the maker's own words (Q17), and the optional blog (§17.5). |
| **5 — Polish & launch** | Performance, a11y, cross-device QA, security review, real payment test | **Started** — all §13.1 security headers set with a nonce-based CSP, /api/health for uptime monitoring. Remaining: screen-reader pass, Lighthouse run, cross-device QA on a real Android phone, and the real payment test (blocked on a gateway). |
